...
class MultiLayerPerceptron(EstBase):
    def __init__(self):
        ...
        {% if layer_num == 1 %}
        hidden_layer_last = utils.perceptron(self.input_data, self.input_dim, self.hidden_dims[0], \
        ...
        {% else %}
        hidden_layer_{{ i + 1 }} = utils.perceptron(hidden_layer_{{ i }}, self.hidden_dims[{{i - 1}}], \
                                                      self.hidden_dims[{{i}}], 'hidden_{}'.format({{i + 1}}), self.activations[{{i}}])
        {% endif %} {% endfor %} {% endif %}
